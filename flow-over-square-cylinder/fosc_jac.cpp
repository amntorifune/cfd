...
// other constants
const double dx    = 32.0 / NCX;
const double dy    = 16.0 / NCY;
...
double BC(int index, int variable, double i, double k, double t) {
    ...
    double uinflow = - (1.0 / 64) * UINFLOW * (y - 16.0) * (y - 0.0);
...
void setupGeom(int ff[NNX][NNY], int bf[NNX][NNY][2]) {
    ...
    // the square cylinder
    for (int i = cylinder[0][_x]; i < cylinder[1][_x]; i ++) {
        for (int k = cylinder[0][_y]; k < cylinder[1][_y]; k ++) {
            ff[i][k] = NONFLUID;
        }
    }
    ...
    for (int i = cylinder[0][_x]; i < cylinder[1][_x]; i ++) {
        bf[i][cylinder[0][_y] - 1][_y] = WALL;
        bf[i][cylinder[1][_y] - 1][_y] = WALL;
    }
    for (int k = cylinder[0][_y]; k < cylinder[1][_y]; k ++) {
        bf[cylinder[0][_x] - 1][k][_x] = WALL;
        bf[cylinder[1][_x] - 1][k][_x] = WALL;
    }
}
/**********************************************************************************/
...
void o2fo(double u[NNX][NNY][2], double p[NNX][NNY], double uF[NNX][NNY][2], double div[NNX][NNY]) {
    ...
    char   fname[128];
    double xpos, ypos;
    sprintf(fname, "Re%d@c.jac.csv", int(Re));
    fo = fopen(fname, "w+t");
    ...
    sprintf(fname, "Re%d@fx.jac.csv", int(Re));
    fo = fopen(fname, "w+t");
    if ( fo == NULL ) {
        printf("\nERROR when opening file\n");
    }
    ...
    sprintf(fname, "Re%d@fy.jac.csv", int(Re));
    fo = fopen(fname, "w+t");
    if ( fo == NULL ) {
        printf("\nERROR when opening file\n");
    }
