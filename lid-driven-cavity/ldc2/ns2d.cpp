...
int poisson(
    ...
    while (con == false) {
        R   = 0;
        con = true;
        #pragma acc kernels loop independent reduction(+:R) collapse(2) present(pi, piN, uF, u, ff, bf, BT) copy(R)
        for (int i = WBOUND; i < EBOUND; i ++) {
            for (int k = SBOUND; k < NBOUND; k ++) {
                if (ff[i][k] == FLUID && (i + k) % 2 == BLACK) {
                    ...
                    picp = pic0 + OMEGA * (psi_ - ae1_ * pie1 - aw1_ * piw1 - an1_ * pin1 - as1_ * pis1 - ac0_ * pic0) / ac0;
                    res  = picp - pic0;
                    R    = R + res * res;
        ...
        for (int i = WBOUND; i < EBOUND; i ++) {
            for (int k = SBOUND; k < NBOUND; k ++) {
                if (ff[i][k] == FLUID && (i + k) % 2 == RED) {
                    ...
                    k_   = 1;
                    k_   = k_ - OMEGA * (ae1 * ee1d + aw1 * ew1d + an1 * en1d + as1 * es1d) / ac0;
                    k_   = k_ + OMEGA * (ae1 * ee1n + aw1 * ew1n + an1 * en1n + as1 * es1n) / ac0;

                    picp = (pic0 + OMEGA * (psi_ - ae1_ * pie1 - aw1_ * piw1 - an1_ * pin1 - as1 * pis1 - ac0_ * pic0)) / k_;
                    R    = R + res * res;
...
void ns2d(
    ...
    while(t < Tr) {
        ...
        while(diver > E) {
            initPi(pi);
            it    = poisson(pi, piN, uF, u, ff, bf, r, t);
...
int main(int argc, char ** argv) {
    ...
    setupGeom(FF, BF);
    #pragma acc enter data copyin(U, UA, UF, UFN, Pi, PiN, P, FF, BF, BT, DIV)
    init(U, UF, P, Pi, FF, BF);
    ns2d(U, UA, UF, UFN, Pi, PiN, P, FF, BF, DIV);
