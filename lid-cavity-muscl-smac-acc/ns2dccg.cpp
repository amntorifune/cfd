...
void applyBCPr(double pr[NN][NN]) {
    #pragma acc kernels loop present(pr)
    for (int i = SWBOUND; i < NEBOUND; i ++) {
        pr[i][NEBOUND    ] = pr[i][NEBOUND - 1] + DPBCn * dd;
        pr[i][SWBOUND - 1] = pr[i][SWBOUND    ] - DPBCs * dd;
    }
    #pragma acc kernels loop present(pr)
    for (int k = SWBOUND; k < NEBOUND; k ++) {
        pr[NEBOUND    ][k] = pr[NEBOUND - 1][k] + DPBCe * dd;
        pr[SWBOUND - 1][k] = pr[SWBOUND    ][k] - DPBCw * dd;
    }
}
