...
//  outflow boundary : (u,v,w) = (outflow), dp/dx = 0
    ...
    for (int j = J1; j <= J2; j ++) {
        for (int k = K1; k <= K2; k ++) {
            FFU[I2][j][k][x] = BOUNDARY;

    //  apply in outflow correction to velocity

            FFP[I2][j][k][x] = BOUNDARY;
...
//  top boundary : (u,v,w) = (slip wall), dp/dz = 0
    ...
    for (int i = I1; i <= I2; i ++) {
        for (int j = J1; j <= J2; j ++) {
            FFU[i][j][K2][z]    = BOUNDARY;

    //  apply slip wall condition to velocity

            FFP[i][j][K2][z]    = BOUNDARY;
...
//  north boundary : (du/dy,v,dw/dy) = (0,0,0), dp/dy = 0
    ...
    for (int i = I1; i <= I2; i ++) {
        for (int k = K1; k <= K2; k ++) {
            FFU[i][J2][k][y]    = BOUNDARY;

    //  apply slip wall condition to velocity

            FFP[i][J2][k][y]    = BOUNDARY;
...
//  south boundary : (du/dy,v,dw/dy) = (0,0,0), dp/dy = 0
    ...
    for (int i = I1; i <= I2; i ++) {
        for (int k = K1; k <= K2; k ++) {
            FFU[i][J1 - 1][k][y]    = BOUNDARY;

    //  apply slip wall condition to velocity

            FFP[i][J1 - 1][k][y]    = BOUNDARY;
...
//  cube top face : (u,v,w) = (0,0,0), dp/dz = [dp/dz]wall
    ...
    for (int i = I3; i <= I4; i ++) {
        for (int j = J3; j <= J4; j ++) {
            FFU[i][j][K4][z]    = BOUNDARY;
            BBU[i][j][K4][z][u] = 0.0;
            BBU[i][j][K4][z][v] = 0.0;
            BBU[i][j][K4][z][w] = 0.0;

            FFP[i][j][K4][z]    = BOUNDARY;
...
//  cube east face : (u,v,w) = (0,0,0), dp/dx = [dp/dx]wall
    ...
    for (int j = J3; j <= J4; j ++) {
        for (int k = K3; k <= K4; k ++) {
            FFU[I4][j][k][x]    = BOUNDARY;
            BBU[I4][j][k][x][u] = 0.0;
            BBU[I4][j][k][x][v] = 0.0;
            BBU[I4][j][k][x][w] = 0.0;

            FFP[I4][j][k][x]    = BOUNDARY;
...
//  cube north face : (u,v,w) = (0,0,0), dp/dy = [dp/dy]wall
    ...
    for (int i = I3; i <= I4; i ++) {
        for (int k = K3; k <= K4; k ++) {
            FFU[i][J4][k][y]    = BOUNDARY;
            BBU[i][J4][k][y][u] = 0.0;
            BBU[i][J4][k][y][v] = 0.0;
            BBU[i][J4][k][y][w] = 0.0;

            FFP[i][J4][k][y]    = BOUNDARY;
