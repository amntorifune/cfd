...
const double UINFLOW   = 1;
const double VINFLOW   = 0;
const double PINFLOW   = 0;
const double PIINFLOW  = 0;
const double UOUTFLOW  = 0;
const double VOUTFLOW  = 0;
const double POUTFLOW  = 0;
const double PIOUTFLOW = 0;
...
//    D  N       D  N       D  N       D  N
    ...
    {{1, 0}   , {1, 0}   , {0, 1}   , {0, 1}   }, // in-flow
    {{0, 1}   , {0, 1}   , {1, 0}   , {1, 0}   }  // out-flow

//  {NONE     , NONE     , NONE     , NONE     }, // no boundary
//  {DIRICHLET, DIRICHLET, NEUMANN  , NEUMANN  }, // wall
//  {DIRICHLET, DIRICHLET, NEUMANN  , NEUMANN  }, // in-flow
//  {NEUMANN  , NEUMANN  , DIRICHLET, DIRICHLET}  // out-flow
};

double BC(int index, int variable, double i, double k, double t) {
    double y       = (k - SBOUND) * dy + 0.5 * dy;
    double uinflow = - 16 * UINFLOW * (y - 1) * (y - 0.5);

    double bc[4][4] = {
    //   u         v         p         π
        {0.0     , 0.0     , 0.0     , 0.0      }, // no boundary
        {UWALL   , VWALL   , PWALL   , PIWALL   }, // wall
        {uinflow , VINFLOW , PINFLOW , PIINFLOW }, // in-flow
        {UOUTFLOW, VOUTFLOW, POUTFLOW, PIOUTFLOW}  // out-flow
    };
...
void setupGeom(int ff[NNX][NNY], int bf[NNX][NNY][2]) {
    ...
    // the step
    for (int i = step[0][_x]; i < step[1][_x]; i ++) {
        for (int k = step[0][_y]; k < step[1][_y]; k ++) {
            ff[i][k] = NONFLUID;
        }
    }
    ...
    for (int k = step[1][_y]; k < NBOUND; k ++) {
        bf[WBOUND - 1][k][_x] = INFLOW;
    }
    // out-flow
    for (int k = SBOUND; k < NBOUND; k ++) {
        bf[EBOUND - 1][k][_x] = OUTFLOW;
    }
    // upper wall
    for (int i = WBOUND; i < EBOUND; i ++) {
        bf[i][NBOUND - 1][_y] = WALL;
    }
    // bottom wall
    for (int i = step[1][_x]; i < EBOUND; i ++) {
        bf[i][SBOUND - 1][_y] = WALL;
    }
    // top face of the step
    for (int i = step[0][_x]; i < step[1][_x]; i ++) {
        bf[i][step[1][_y] - 1][_y] = WALL;
    }
    // side face of the step
    for (int k = step[0][_y]; k < step[1][_y]; k ++) {
        bf[step[1][_x] - 1][k][_x] = WALL;
    }
}
/**********************************************************************************/
...
double interpolR(double l1, double r1, double r2, int eps) {
    double dm = r1 - l1;
    double dp = r2 - r1;
    return r1 - eps * 0.25 * albada2(R(dm, dp)) * ((1 - KAPPA) * dp + (1 + KAPPA) * dm);
}
...
double interpolL(double l2, double l1, double r1, int eps) {
    double dm = l1 - l2;
    double dp = r1 - l1;
    return l1 + eps * 0.25 * albada2(R(dm, dp)) * ((1 - KAPPA) * dm + (1 + KAPPA) * dp);
}
        ...
        for (int i = WBOUND; i < EBOUND; i ++) {
            for (int k = SBOUND; k < NBOUND; k ++) {
                if (ff[i][k] == FLUID && (i + k) % 2 == BLACK) {
                    ...
                    double ae1_, aw1_, ac0_, an1_, as1_; // coefficient with embedded boundary condition
                    double psi, psi_, res;               //
                    int    ee1, ew1, en1, es1;           // first level boundary indicator
                    int    ee1d, ew1d, en1d, es1d;       // ditichlet condition indicator
                    int    ee1n, ew1n, en1n, es1n;       // neumann condition indicator
                    double bce, bcw, bcn, bcs;           // boundary condition values for π at each direction
                    bE1(i, k, bf, ee1, ew1, en1, es1);
                    ...
                    ee1d = BT[ee1][_pi][_dirichlet];
                    ew1d = BT[ew1][_pi][_dirichlet];
                    en1d = BT[en1][_pi][_dirichlet];
                    es1d = BT[es1][_pi][_dirichlet];
                    ee1n = BT[ee1][_pi][_neumann  ];
                    ew1n = BT[ew1][_pi][_neumann  ];
                    en1n = BT[en1][_pi][_neumann  ];
                    es1n = BT[es1][_pi][_neumann  ];
                    ...
                    ac0_ = ac0;
                    ac0_ = ac0_ - ae1 * ee1d - aw1 * ew1d - an1 * en1d - as1 * es1d;
                    ac0_ = ac0_ + ae1 * ee1n + aw1 * ew1n + an1 * en1n + as1 * es1n;
                    ae1_ = ae1 * (1 - ee1d) * (1 - ee1n);
                    aw1_ = aw1 * (1 - ew1d) * (1 - ew1n);
                    an1_ = an1 * (1 - en1d) * (1 - en1n);
                    as1_ = as1 * (1 - es1d) * (1 - es1n);

                    bce  = BC(ee1, _pi, i + 0.5, k      , t);
                    bcw  = BC(ew1, _pi, i - 0.5, k      , t);
                    bcn  = BC(en1, _pi, i      , k + 0.5, t);
                    bcs  = BC(es1, _pi, i      , k - 0.5, t);
                    ...
                    psi_ = psi;
                    psi_ = psi_ - 2 * ae1 * ee1d * bce;
                    psi_ = psi_ - 2 * aw1 * ew1d * bcw;
                    psi_ = psi_ - 2 * an1 * en1d * bcn;
                    psi_ = psi_ - 2 * as1 * es1d * bcs;
                    psi_ = psi_ - ae1 * ee1n * dx * bce;
                    psi_ = psi_ + aw1 * ew1n * dx * bcw;
                    psi_ = psi_ - an1 * en1n * dy * bcn;
                    psi_ = psi_ + as1 * es1n * dy * bcs;

                    pica = (psi_ - ae1_ * pie1 - aw1_ * piw1 - an1_ * pin1 - as1_ * pis1) / ac0_;
                    res  = OMEGA * (pica - pic0);
                    pic0 = pic0 + res;
                    R    = R + res * res;
...
void ns2d(
    ...
    while(t < Tr) {
        ...
        while(diver > E) {
            ...
            printf("\rt = %5.8lf, poi = %5d, it = %5d, div = %5.8lf, res = %5.8lf, max div = %5.8lf", t, it, it2, diver, r, maxdiver);
            fflush(stdout);
            it2 += 1;
        }
        t += dt;
